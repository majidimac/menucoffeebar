...
// PERFORMANCE PATTERN: Reuse Intl formatters as global constants to avoid overhead in render loops
...
const formatPrice = (price: number) => {
  return CURRENCY_FORMATTER.format(price) + ' تومان';
};
 ...
 * 1. Image Optimization: Reduced bgImageUrl width from 1200px to 600px and branding image to 600px.
 *    Expected Impact: Significant reduction in image payload size and faster decoding.
...
const bgImageUrl = "https://images.unsplash.com/photo-1442512595331-e89e73853f31?q=80&w=600&auto=format&fit=crop";
...
// --- PERFORMANCE PATTERN: Hoist sub-components outside main render loop ---
...
}> = ({ loginForm, setLoginForm, handleAdminLogin, loginError, setView }) => (
...
}> = ({ orders, setView, markOrderAsCompleted, deleteOrder }) => (
              ...
              <div className="p-8 flex flex-col md:flex-row justify-between items-start md:items-center border-b border-gray-50 gap-6">
                <div>
                  ...
                  <div className="flex items-center gap-2 text-zinc-400 text-sm">
                    ...
                    <span>{TIME_FORMATTER.format(new Date(order.timestamp))}</span>
                    <span className="mx-1">•</span>
                    <span>{DATE_FORMATTER.format(new Date(order.timestamp))}</span>
                  </div>
                </div>
