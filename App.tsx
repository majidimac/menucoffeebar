...
// Branding Section component - Memoized to prevent re-renders when cart/view changes
...
const AdminLogin = React.memo(({ loginForm, setLoginForm, handleAdminLogin, loginError, setView }: AdminLoginProps) => (
...
const AdminDashboard = React.memo(({ orders, setView, markOrderAsCompleted, deleteOrder }: AdminDashboardProps) => (
...
const App: React.FC = () => {
  ...
  const submitOrder = () => {
    ...
    setOrders(prev => {
      ...
      localStorage.setItem('cafe_gandom_orders', JSON.stringify(updated));
      return updated;
    });
    setCart([]);
    setIsCartOpen(false);
  ...
  // Bolt Optimization: Wrap event handlers in useCallback to ensure
  // stable references and truly effective memoization for child components.
